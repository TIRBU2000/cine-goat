...
if (tmdbKey) {
  fetch(
    `https://api.themoviedb.org/3/movie/popular?api_key=${tmdbKey}&language=fr-FR`,
  )
    .then((res) => {
    ...
    .then((data) => {
      if (data.results && data.results.length > 0) {
        printLog(
          `Connexion TMDB réussie ! (Film N°1 actuel : "${data.results[0].title}")`,
          true,
        );
      }
    })
    .catch(() =>
      printLog(
        "L'API TMDB a refusé la connexion (Vérifie ta clé VITE_TMDB_API_KEY)",
        false,
      ),
    );
}*/
